...
def run_compliance():  # noqa: C901
    ...
    if Path(".gitlab/ci/security.yml").exists():
        with open(".gitlab/ci/security.yml") as f:
            content = f.read()
    ...
    if final_score < 100:
        ...
        if not Path("bruno").exists():
            print("❌ Bruno — API collection must be present in the bruno/ directory.")
